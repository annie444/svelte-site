<script lang="ts">
	...
	const { config, addLayer, removeLayer } = getContext(contextKey) as {
		config: Config;
	...
	const coord = spring<number>(undefined, config);
	// and one to hold intersecting progress
	const progress = spring<number>(undefined, { ...config, precision: 0.001 });
	// layer height
	let height: number;

	const layer: Layer = {
		setPosition: (scrollTop: number, sectionHeight: number, disabled: boolean) => {
			if (disabled) {
		...
		setHeight: (sectionHeight: number) => {
